...
# list all SHAs
ls at_py_2020/20211130_*.gz | parallel "gzcat {} | go run preprocess_at_py_2020.go -h"
cat at_py_2020/athena_py.*.txt > at_py_2020_sha.txt

# save content in line-based text format
ls at_py_2020/20211130_*.gz | parallel "gzcat {} | go run preprocess_bq_py_2016.go"
...
files_with_content as (
    SELECT
        repo_id,
        bigcode_prod_files.commit_sha as commit_sha,
        path,
        size,
        bigcode_prod_files.content_sha as content_sha,
        content
    FROM filtered_small_python
    JOIN bigcode_prod_files
    ON filtered_small_python.content_sha = bigcode_prod_files.content_sha
)
SELECT
...
JOIN bigcode_prod_repositories
ON files_with_content.repo_id = bigcode_prod_repositories.repo_id AND files_with_content.commit_sha = bigcode_prod_repositories.commit_sha
