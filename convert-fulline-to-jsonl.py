...
GENERATED_HEADERS = [
    "Generated by the gRPC", "Generated by the protocol buffer",
    "Autogenerated by Thrift", "was automatically generated by SWIG",
    "giant blob of binary data",
]
NEWLINE_REPLACEMENT="Ċ" # \n + 100 == \u0100a  # printf '\u010a\n' | hexdump -C
...
def process_single_repo(repo_path: str, data_dir: str) -> DefaultDict[str, int]:
    ...
    def process_file(org_repo, metadata, py_filepath, out):
        ...
        if data["size"] > FILE_SIZE_LIMIT:
            raise Skip(">1mb files") #return # skip big files

        with open(py_filepath, "rb") as f:
            bytes = f.read()
            content = bytes.decode("utf8")
            if len(content) == 0:
                raise Skip("empty") #return # skip empty files

            if any(s in content[:256] for s in GENERATED_HEADERS):
                raise Skip("generated") #return # skip genererate files
            ...
            if avg_len > 100 or max_len > 1000:
                raise Skip("lines long")

            data["content"] = NEWLINE_REPLACEMENT.join(lines)
